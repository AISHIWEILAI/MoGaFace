...
class CameraInfo(NamedTuple):
    ...
    all_view_fig_path: Optional[dict]
    image_path: str
    image_name: str
    width: int
    height: int
    ...
    landmark2d: Optional[np.array] = np.zeros((1, 68))
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames)):
            ...
            try:
                material = file_path.split('/')[-3].split('_')[2]
            except:
                material = '1'
            cam_idx = frame['camera_id']
            frame_idx = file_path.split('/')[-1].split('.')[0].split('_')[0]

            # all views
            print("######### all views ###########")
            all_view_fig_path = {file_path.split('/')[-1].split('.')[0].split('_')[1]: [os.path.join(path,
                                                                                                     os.path.dirname(
                                                                                                         file_path),
                                                                                                     f'{frame_idx}_{multi_view_cam[cam]}.png')
                                                                                        for cam in
                                                                                        multi_view_cam.keys()]}
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, bg=bg, image=image,
                                        all_view_fig_path=all_view_fig_path,
                                        image_path=image_path, image_name=image_name,
                                        width=width, height=height,
                                        timestep=timestep, camera_id=camera_id, landmark2d=landmark2d
                                        ))
    return cam_infos
...
def readMeshesFromTransforms(path, transformsfile):
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames)):
            ...
            flame_param = dict(np.load(os.path.join(path,
                                                    frame['flame_param_path']),
                                       allow_pickle=True))
            mesh_infos[frame["timestep_index"]] = flame_param
    return mesh_infos

def readDynamicNerfInfo(path, white_background,
                        eval, extension=".png", target_path=""):
    print("Reading Training Transforms")
    if target_path != "":
        train_cam_infos = readCamerasFromTransforms(target_path,
    ...
    train_mesh_infos = readMeshesFromTransforms(path,
                                                "transforms_train.json")
    if target_path != "":
        print("Reading Target Meshes (Training Division)")
        tgt_train_mesh_infos = readMeshesFromTransforms(target_path,
                                                        "transforms_train.json")
    else:
    ...
    if target_path != "":
        val_cam_infos = readCamerasFromTransforms(target_path,
                                                  "transforms_val.json",
                                                  white_background,
                                                  extension)
    else:
        val_cam_infos = readCamerasFromTransforms(path,
                                                  "transforms_val.json",
                                                  white_background,
                                                  extension)
    ...
    print("Reading Test Transforms")
    if target_path != "":
        test_cam_infos = readCamerasFromTransforms(target_path,
    ...
    test_mesh_infos = readMeshesFromTransforms(path,
                                               "transforms_test.json")
    if target_path != "":
        print("Reading Target Meshes (Test Division)")
        tgt_test_mesh_infos = readMeshesFromTransforms(target_path,
                                                       "transforms_test.json")
    else:
        tgt_test_mesh_infos = {}
