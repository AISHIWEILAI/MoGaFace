...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self.training_setup(training_args, device=torch.device('cuda:0'))
    ...
    def get_scaling(self):
        if self.binding is None:
            ...
            return scaling[self.binding] * self.face_scaling[self.binding]
    ...
    def get_rotation(self):
        if self.binding is None:
            ...
            return quat_xyzw_to_wxyz(quat_product(quat_wxyz_to_xyzw(face_orien_quat),
                                                  quat_wxyz_to_xyzw(rot[self.binding])))
    ...
    def create_from_pcd(self, pcd: Optional[BasicPointCloud], spatial_lr_scale: float, device='cpu'):
        self.spatial_lr_scale = spatial_lr_scale

        if pcd == None:
            assert self.binding is not None
            num_pts = self.binding.shape[0] #+ self.filled_lip_pts.shape[0]
            fused_point_cloud = torch.zeros((num_pts, 3)).float().to(device)
            fused_color = torch.tensor(np.random.random((num_pts, 3)) / 255.0).float().to(device)
        else:
        ...
        if self.binding is None:
            ...
            scales = torch.log(torch.ones((self.get_xyz.shape[0], 3), device=device))
    ...
    def training_setup(self, training_args, device=torch.device('cuda:0')):
    ...
    def load_ply(self, path, check_info=None, **kwargs):
    ...
    def prune_points(self, mask):
        if self.binding is not None:
            ...
            counter_prune.scatter_add_(0, binding_to_prune,
        ...
        if self.binding is not None:
            # Toyota Motor Europe NV/SA and its affiliated companies retain all intellectual property and proprietary rights in and to the following code lines and related documentation. Any commercial use, reproduction, disclosure or distribution of these code lines and related documentation without an express license agreement from Toyota Motor Europe NV/SA is strictly prohibited.
            self.binding_counter.scatter_add_(0, self.binding[mask],
