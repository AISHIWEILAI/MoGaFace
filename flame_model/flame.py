...
from .lbs import lbs, vertices2landmarks, blend_shapes, batch_rodrigues, rot_mat_to_euler
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        lmk_embeddings = np.load(flame_lmk_embedding_path,
                                 allow_pickle=True,
                                 encoding="latin1"
        )
        ...
        mediapipe_lmk_embedding = np.load(str(Path(__file__).resolve().parent / 'assets/mediapipe/mediapipe_landmark_embedding.npz'), allow_pickle=True, encoding='latin1')
        ...
        self.register_buffer('lmk_bary_coords',
                             torch.from_numpy(lmk_embeddings['static_lmk_bary_coords']).to(self.dtype).float())
        self.register_buffer('dynamic_lmk_faces_idx',
                             torch.from_numpy(np.array(lmk_embeddings['dynamic_lmk_faces_idx']).astype(int)).to(
                                 torch.int64))
        self.register_buffer('dynamic_lmk_bary_coords',
                             torch.from_numpy(np.array(lmk_embeddings['dynamic_lmk_bary_coords'])).to(
                                 self.dtype).float())
        ...
        self.faces_ori = self.faces.clone()
    ...
    def forward(
        ...
        if return_landmarks :
            lmk_faces_idx = self.lmk_faces_idx.unsqueeze(dim=0)[:, :51].expand(batch_size, -1).contiguous()
            lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0)[:, :51].expand(batch_size, -1, -1).contiguous()

            dyn_lmk_faces_idx, dyn_lmk_bary_coords = self._find_dynamic_lmk_idx_and_bcoords(vertices, full_pose,
            ...
            lmk68 = vertices2landmarks( vertices,
                                        self.faces_tensor,
                                        lmk_faces_idx,
                                        lmk_bary_coords,)
            bz = vertices.shape[0]
            ...
            lmk68 = lmk68 + translation[:, None, :]
            landmarks3d = landmarks3d + translation[:, None, :]

            ret_vals.append(lmk68)
            ret_vals.append(landmarks3d)
            return vertices, v_shaped, lmk68, landmarks3d
        return vertices, v_shaped
        # if len(ret_vals) > 1:
        #     return ret_vals
        # else:
        #     return ret_vals[0]
